...
import { useAllAgents, type Agent as ContractAgent } from '@/hooks/useAgentRegistry'
    ...
    Promise.all(
      data.map(async (agent: ContractAgent): Promise<AgentWithMetadata> => {
        ...
        return {
          ...
          basePriceStr: metadata.basePrice || '0'
        }
      })
    )
    .then(setAgents)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="text-center">
            ...
            <p className="text-xl md:text-2xl mb-8 text-primary-100">
              Descubre y Renta agentes de inteligencia artificial en Avalanche
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <button type='button' className="border-2 border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white hover:text-black transition-colors hover:cursor-pointer">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row gap-4 mb-8">
            ...
            <select
              ...
              className="px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-black"
              ...
              <div key={agent.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow">
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <span className="bg-primary-100 text-black px-2 py-1 rounded-full text-xs font-medium">
                      {agent.category}
                    </span>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-lg font-bold text-black">{formatEther(agent.basePrice)} AVAX</span>
                    <span className="text-sm text-gray-500">0 usuarios</span>
                  </div>

                  <Link
                    href={`/agent/${agent.id}`}
                    className="w-full bg-black text-white px-4 py-2 rounded-lg hover:bg-primary-700 transition-colors text-center block"
                  >
                    Ver Detalles
                  </Link>
                </div>
