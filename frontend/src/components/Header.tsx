...
export function Header() {
  ...
  useEffect(() => {
    // Esto asegura que el render ocurra solo en el cliente
    setMounted(true)
  }, [])

  const formatAddress = (address: string) => `${address.slice(0, 6)}...${address.slice(-4)}`

  // Mientras el componente no esté montado, no renderiza nada dependiente del cliente
  if (!mounted) {
    return (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <Link href="/" className="flex items-center space-x-2">
              ...
              <span className="text-xl font-bold text-gray-900">AI Agents Marketplace</span>
            </Link>
          </div>
        </div>
      </header>
